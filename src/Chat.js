...
export default class Chat extends React.Component {
  ...
  senduserreport=async()=>{
    ...
    let data=this.state.data
    var other_user_id = data.other_user_id
    var url = config.baseURL+'user_report_add.php?user_id='+user_id+'&other_user_id='+other_user_id+'&report_type=1';
    consolepro.consolelog('url',url)
    this.setState({loading:true,})
    fetch(url,{
      method: 'GET',
 ...
 btnOpneImageOption = (index) => {
   ...
   this.props.navigation.navigate('chatreport',{'other_user_id':other_user_id,'chat_type':1})
  }, 500);
      // Alert.alert(
      //     //This is title
...
<KeyboardAvoidingView style={styles.container} behavior='padding'>
  ...
  <Loader loading={this.state.loading}/>
   ...
   >
     ...
     <View  style={{backgroundColor:'#f5f4f2',height:'auto',position:'absolute',bottom:0,left:0,right:0}}> 
       <View style={{paddingTop:15,paddingHorizontal:20}}>
           <Text style={{paddingVertical:10,fontSize:15.5,fontFamily:'Piazzolla-Bold',textAlign:'flex-start'}}>{Lang_chg.chataction[config.language]}</Text>
       <TouchableOpacity  onPress={()=>{this.setState({modalVisible:false});this.permmissionsendreport()}}>
                <Text style={{color:'black',fontSize:15,fontFamily:'Piazzolla-SemiBold'}}>{Lang_chg.chatreport[config.language]}</Text>
           </TouchableOpacity>
           <TouchableOpacity style={{paddingVertical:16}}  onPress={()=>{this.setState({modalVisible:false});this.permmissionclearchat()}}>
                <Text style={{color:'black',fontFamily:'Piazzolla-SemiBold',fontSize:14}}>{Lang_chg.chatclear[config.language]}</Text>
           </TouchableOpacity>
           
           <TouchableOpacity style={{paddingBottom:15}} onPress={()=>{this.setState({modalVisible:false})}}>
                <Text style={{color:Colorss.theme1,fontSize:15,fontFamily:'Piazzolla-SemiBold'}}>{Lang_chg.chatcancel[config.language]}</Text>
           </TouchableOpacity>
      </View>   
   ...
   <View style={{paddingVertical:14,backgroundColor:Colorss.theme1,flexDirection:'row',width:'100%',}}>
         <View style={{width:'15%'}}>
           <TouchableOpacity style={{height:30,width:'100%',paddingTop:3}} onPressOut={()=>{this.props.navigation.goBack()}}>
           ...
           <View style={{flexDirection:'row',alignSelf:'center',width:'85%',}}>
               <View style={{flexDirection:'row',width:'80%'}}>
                <Text style={{color:'white',fontSize:17,fontFamily:'Piazzolla-Bold',paddingRight:10}}>{this.state.data.other_user_name}</Text>
                {this.state.data.image==undefined?
                              <Image source={require('./icons/new.png')} style={{width:20,alignSelf:'center',height:20,resizeMode:'contain',}}/>:
                          this.state.data.image=='NA'?
                          <Image source={require('./icons/new.png')} style={{width:20,alignSelf:'center',height:20,resizeMode:'contain'}}/>:
                          this.state.data.image==null?
                          <Image source={require('./icons/new.png')} style={{width:20,alignSelf:'center',height:20,resizeMode:'contain'}}/>:
                            <Image source={{uri:this.state.data.image}} style={{width:30,alignSelf:'center',height:30,borderRadius:15,}}/>
           ...
           <TouchableOpacity style={{width:'100%',alignItems:'flex-end'}} onPressOut={()=>{this.setState({modalVisible:true})}}>
               <Image source={require('./icons/dots.png')} style={{width:6,height:24,resizeMode:'contain'}}/>
           </TouchableOpacity>  
           </View> 
           </View> 
       </View>
              ...
              <View style={{width:'95%',alignSelf:'center',paddingVertical:7,}}>
               {item.userid!=this.state.user_id &&<View style={{alignSelf:'flex-start',width:'70%'}}>
                 <View style={{backgroundColor:'#d7d7d7',paddingVertical:1.5,paddingHorizontal:8,alignSelf:'flex-start',borderTopStartRadius:6,borderTopEndRadius:6,borderBottomLeftRadius:6}}>
                      {item.messageType=='text' && <Text style={{fontSize:14,fontFamily:'Piazzolla-Bold',color:'black'}}>{item.name}</Text>}
                      {item.messageType=='image' && <TouchableOpacity >
                       <Image source={{uri:config.img_url+item.name}} style={{width:BannerWidth*42/100,height:BannerHieght*30/100,borderRadius:5,backgroundColor:Colorss.imagebackcolor}}/>
                       </TouchableOpacity>}
                       <Text style={{fontSize:14,fontFamily:'Piazzolla-SemiBold',color:'gray'}}>{item.time}</Text>
                     </View>
                 </View>}
                 {item.userid==this.state.user_id && <View style={{width:'70%',alignSelf:'flex-end',}}>
                     <View style={{backgroundColor:Colorss.theme1,borderTopStartRadius:6,borderTopEndRadius:6,borderBottomLeftRadius:6,paddingVertical:1.5,paddingHorizontal:8,alignSelf:'flex-end'}}>
                     {item.messageType=='text' && <Text style={{fontSize:14,fontFamily:'Piazzolla-SemiBold',color:'#FFFFFF'}}>{item.name}</Text>}
                      {item.messageType=='image' && <TouchableOpacity >
                       <Image source={{uri:config.img_url+item.name}} style={{width:BannerWidth*42/100,height:BannerHieght*30/100,borderRadius:5,backgroundColor:Colorss.imagebackcolor}}/>
                       </TouchableOpacity>}
                       <Text style={{fontSize:14,fontFamily:'Piazzolla-Medium',color:'#FFFFFF'}}>{item.time}</Text>
                     </View>
...
{this.state.data.blockstatus!='no' &&
    <TouchableOpacity onPress={()=>{alert('unblock')}}>
     <View style={{alignSelf:'center',paddingVertical:10}}>
        <Text style={{color:Colorss.theme1,fontSize:15,textAlign:'center',textAlignVertical:'center'}}>you blocked this user.tap to unblock</Text>
     </View>
    </TouchableOpacity>
 }
{this.state.data.blockstatus=='no' &&  <View style={{paddingHorizontal:20,width:'100%',alignSelf:'center',flexDirection:'row',justifyContent:'space-between'}}>
       ...
       <TouchableOpacity style={{height:30,width:'10%',alignSelf:'center',paddingTop:4,}} onPress={()=>{this.btnOpneImageOption()}}>
        ...
        <TextInput
                 ...
                 placeholderTextColor={Colorss.theme1}
                ...
                style={{width:'77%',paddingRight:7,fontFamily:'Piazzolla-SemiBold',paddingVertical:8,color:'black'}}
            />
...
return(
 ...
 <Loader loading={this.state.loading}/>
   ...
   >
     ...
     <View  style={{backgroundColor:'#f5f4f2',height:'auto',position:'absolute',bottom:0,left:0,right:0}}> 
       <View style={{paddingTop:15,paddingHorizontal:20}}>
           <Text style={{paddingVertical:10,fontSize:16,fontFamily:'Piazzolla-Bold'}}>{Lang_chg.chataction[config.language]}</Text>
       <TouchableOpacity  onPress={()=>{this.setState({modalVisible:false});this.permmissionsendreport()}}>
                <Text style={{color:'black',fontSize:15,fontFamily:'Piazzolla-SemiBold'}}>{Lang_chg.chatreport[config.language]}</Text>
           </TouchableOpacity>
           <TouchableOpacity style={{paddingVertical:16}}  onPress={()=>{this.setState({modalVisible:false});this.permmissionclearchat()}}>
           ...
           <TouchableOpacity style={{paddingBottom:15}} onPress={()=>{this.setState({modalVisible:false})}}>
                <Text style={{color:'red',fontSize:15,fontFamily:'Piazzolla-SemiBold'}}>{Lang_chg.chatcancel[config.language]}</Text>
           </TouchableOpacity>
      </View>   
          ...
          <TouchableOpacity style={{height:30,width:'10%',alignSelf:'center',paddingTop:4,}} onPress={()=>{this.btnOpneImageOption()}}>
           ...
           <TextInput
                   ...
                   blurOnSubmit={true}
                    scrollEnabled={true}
                    onSubmitEditing={()=>{this.setState({bottom:0})}}
                   style={{width:'77%',paddingRight:6,fontFamily:'Piazzolla-SemiBold',paddingVertical:8,color:'black'}}
               />
              {this.state.chatmsg.length>0 &&    <TouchableOpacity style={{width:'13%',alignSelf:'center',alignItems:'flex-end'}} onPress={()=>{this.sendmessagebtn()}}>
            <Image source={require('./icons/send.png')} style={{width:25,height:15,resizeMode:'contain'}}/>
