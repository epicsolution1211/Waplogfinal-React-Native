...
export default class Mapsearch extends Component{
   constructor(props) {
        ...
        this.state = {
           ...
           modalVisible1:false,
           isConnected:true,
           signuplocation:this.props.route.params.signuplocation,
      ...
      getlatlong=async()=>{
       ...
       getalldata=(position)=>{
          ...
          if(selleraddress1!='NA')
             {
                this.setState({latitude:selleraddress1.latitude,longitude:selleraddress1.longitude})
             }
           else if(filteraddress!='NA')
             {
                this.setState({latitude:filteraddress.latitude,longitude:filteraddress.longitude})
             }
          else{
  ...
  //        Keyboard.dismiss()
  //      if (selleraddress=='NA') {
  //          msgProvider.toast('Please select address')
  //          return false;
  //       }
  //       let user_id=userdata.user_id
  //       let location=selleraddress.address
  //       let latitude=selleraddress.latitude
  //       let longitude=selleraddress.longitude
  //       if(this.state.isConnected==true)
  //       {
  //          this.setState({loading:true});
  //    let url=config.baseURL+"edit_change_location.php?user_id="+user_id+'&location='+location+'&latitude='+latitude+'&longitude='+longitude;
  //      console.log(url)
  ...
  //               this.setState({longitude:obj.user_detail.longitude,latitude:obj.user_detail.latitude})
        ...
        .then((resp) => {
         ...
         let  data2={'latitude':details.geometry.location.lat,'longitude':details.geometry.location.lng,'address':details.formatted_address,'city':city}
             let data3={'lat':details.geometry.location.lat,'lon':details.geometry.location.lng,'isp':details.formatted_address,'city':city}
               ...
               selleraddress=data2
               filteraddress=data2
               console.log('this.state.signuplocation',this.state.signuplocation)
              
               if(this.state.signuplocation=='singuplocation')
               {
                selleraddress1=data2
                console.log('singuplocation',selleraddress1)
               }
