...
import { Text, StyleSheet, FlatList,ScrollView,Keyboard,ImageBackground, View, Image, TouchableOpacity, TextInput } from 'react-native'
...
export default class Favoritemovie extends Component {
    ...
    useralldetaile = async() => {
        ...
        {
            msgProvider.toast(Lang_chg.validationmovie[config.language],'center')
            return false
        }
        if(this.state.isConnected==true)
        {
          ...
          data.append('edit_type','fav_movie')
          data.append('edit_text',this.state.movie)
 ...
 userdata1=async()=>{
     let userdata=await localStorage.getItemObject('user_arr')
     if(userdata.fav_movie!='NA')
     {
         this.setState({movie:userdata.fav_movie})
     }
     }
   backpress=()=>{
        this.props.navigation.goBack()
              ...
              <ScrollView style={{width:'90%',alignSelf:'center'}} showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag' keyboardShouldPersistTaps='always'>
                ...
                <View style={{ paddingVertical: 30 }}>
                    <View style={{ width: '100%', }}>
                        <ProgressBar style={{ height: 5 }} progress={0.9} color={Colors.red800} />
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%',}} colors={Colorss.basecolor}>
                    <View style={{ marginTop: 5 }}>
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontWeight: 'bold', color: 'white' }}>9/10</Text>
                        <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35, resizeMode: 'contain' }} source={require('./icons/favorite_moview.png')}>
                        </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontWeight: 'bold', color: 'white' }}>{Lang_chg.tittlemovie[config.language]}</Text>
                    </View>
                    ...
                    <TextInput 
                     placeholder={Lang_chg.entermovie[config.language]}
                     ...
                     onChangeText={(txt)=>{this.setState({movie:txt})}}
