...
import { Text, StyleSheet, FlatList,ScrollView, ImageBackground,Keyboard, View, Image, TouchableOpacity, TextInput } from 'react-native'
...
export default class Favoritebook extends Component {
    ...
    userdata1=async()=>{
        ...
        if(userdata.fav_book!='NA')
        {
            this.setState({book:userdata.fav_book})
        }
        }
    useralldetaile = async() => {
        ...
        {
            msgProvider.toast(Lang_chg.validationbook[config.language],'center')
            return false
        }
        if(this.state.isConnected==true)
        {
          ...
          data.append('edit_type','fav_book')
          data.append('edit_text',this.state.book)
              ...
              <ScrollView style={{width:'90%',alignSelf:'center'}} showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag' keyboardShouldPersistTaps='always'>
                ...
                <View style={{ paddingVertical: 30 }}>
                    <View style={{ width: '100%', }}>
                        <ProgressBar style={{ height: 5 }} progress={0.8} color={Colors.red800} />
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%',  }} colors={Colorss.basecolor}>
                    <View style={{ marginTop: 5 }}>
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontFamily:'Piazzolla-Bold', color: 'white' }}>8/10</Text>
                        <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35, resizeMode: 'contain' }} source={require('./icons/favoritebookww.png')}>
                        </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontFamily:'Piazzolla-Bold', color: 'white' }}>{Lang_chg.tittlebook[config.language]}</Text>
                    </View>
                    ...
                    <TextInput 
                     placeholder={Lang_chg.enterbook[config.language]}
                     ...
                     onChangeText={(txt)=>{this.setState({book:txt})}}
