...
import { Text, StyleSheet, FlatList ,ImageBackground, Keyboard,View, Image, TouchableOpacity, TextInput, ScrollView } from 'react-native'
...
export default class Favoritetvshow extends Component {
    ...
    userdata1=async()=>{
        ...
        if(userdata.fav_tv_show!='NA')
        {
            this.setState({tvshow:userdata.fav_tv_show})
        }
        }
    useralldetaile = async() => {
        ...
        Keyboard.dismiss()
        if(this.state.tvshow.length<=0)
        {
            msgProvider.toast(Lang_chg.validationtvshow[config.language],'center')
            return false
        }
        if(this.state.isConnected==true)
        {
          ...
          data.append('edit_type','fav_tv_show')
          data.append('edit_text',this.state.tvshow)
              ...
              <ScrollView style={{width:'90%',alignSelf:'center'}} showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag' keyboardShouldPersistTaps='always'>
                ...
                <View style={{ paddingVertical: 30 }}>
                    <View style={{ width: '100%', }}>
                        <ProgressBar style={{ height: 5 }} progress={0.7} color={Colors.red800} />
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%',  }} colors={Colorss.basecolor}>
                    <View style={{ marginTop: 5 }}>
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontWeight: 'bold', color: 'white' }}>7/10</Text>
                        <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35, resizeMode: 'contain' }} source={require('./icons/favorite_moview.png')}>
                        </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontWeight: 'bold', color: 'white' }}>{Lang_chg.tittletvshow[config.language]}</Text>
                    </View>
                    ...
                    <TextInput 
                     placeholder={Lang_chg.entertvshow[config.language]}
                     ...
                     onChangeText={(txt)=>{this.setState({tvshow:txt})}}
