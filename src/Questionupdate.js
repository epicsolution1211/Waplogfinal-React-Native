...
export default class Questionupdate extends Component {
  state = {
         modalVisible: false,
         question_arr:'NA',
         updatequestion:this.props.route.params.updatequestion,
         answer_arr:'NA',
         answer_id:0,
   ...
   getqustionanswer = async() => {
    console.log('updatequestion',this.state.updatequestion)
    }
    ...
    renderitem = ({ item }) => {
        ...
        return (
            <View>
                <TouchableOpacity onPress={()=>{this.setState({'question_id':item.question_id, answer_arr:item.answer_arr}); this.setState({ modalVisible: true})}} style={{justifyContent:'center',alignItems:'center', borderRadius: 10, backgroundColor: 'white', paddingHorizontal: 15, marginVertical: 7,width:'100%', height: 70 }}>
                 <View>
    ...
    backpress=()=>{
      ...
      addanswerquestion = async() => {
        ...
        let user_id=userdata.user_id
         ...
         {
            msgProvider.toast('Please select any question','center')
           return false
         }
         if(this.state.answer_id==0)
         {
            msgProvider.toast('Please select  anwser of selected question','center')
           return false
         }
            ...
            <ScrollView>
             <View style={{ paddingHorizontal: 15}}>
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%',  }} colors={Colorss.basecolor}>
                    ...
                    <TouchableOpacity onPress={()=>{this.addanswerquestion()}}>
                    <Text style={{  fontSize: 16, fontWeight: 'bold', color: 'white',width:120,height:30 }}>save</Text>
                    </TouchableOpacity>
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontWeight: 'bold', color: 'white' }}>1/10</Text>
                        <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35,resizeMode: 'contain' }} source={require('./icons/question.png')}>
                        </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontWeight: 'bold', color: 'white' }}>Question</Text>
                        <Text style={{ margin:20, marginTop: 10, alignSelf: 'center', fontSize: 18, color: 'white' }}>Answer upto 3 question, slide from the crowds and show who you are.</Text>
                    </View>
                    ...
                    {/* <View style={{ alignSelf: 'center',position:'absolute',bottom:15}}>
                        <Text  style={{   fontSize: 16, fontWeight: 'bold', color: 'white' }}>Clear</Text>
                    </View> */}
      </LinearGradient>
                </View>
              ...
              <View style={{  backgroundColor: Colorss.whiteColor, position: 'absolute', bottom: 0, width: '100%', }}>
                ...
                <TouchableOpacity onPress={()=>{ if(this.state.answer_id==0){
                 ...
                 return false};this.setState({modalVisible:false})}}>
                  <Text style={{textAlign:'center',alignSelf:'center',  fontWeight: 'bold', fontSize: 24, color: Colorss.blackColor }}>Done</Text>
                  </TouchableOpacity>
                ...
                <View style={{paddingHorizontal:40, justifyContent: 'center', alignItems: 'center',marginTop:10  }}>
                  <Text style={{textAlign:'center',alignSelf:'center',  fontWeight: 'bold', fontSize: 24, color: Colorss.blackColor }}>What should be your ideal first date?</Text>
