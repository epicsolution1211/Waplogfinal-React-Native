...
export default class Paypalpayment extends Component {
    constructor(props){
        super(props)
        this.state={
             url_paypal:this.props.route.params.url_paypal,
    ...
    if(webViewState.loading==false)
    {
     ...
     if(t == 'paypal_success.php'){
       ...
       let cartcount=0
        ...
        msgProvider.toast(Lang_chg.Payment_successful[config.language],'center');
      
      //   this.props.navigation.navigate('Succesfullorder',{'order_id':order_id})
    }
     else if(t=='paypal_cancel.php'){
         msgProvider.toast(Lang_chg.Payment_un_successful[config.language],false);
         this.props.navigation.navigate('Profile')
       }
    }
  }}
