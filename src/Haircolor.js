...
export default class Haircolor extends Component {
   constructor(props){
       super(props)
       this.state={
           hair_arr:this.props.route.params.hair_arr,
           id:'',
           isConnected:true,
           loading:false,
   ...
   userdata1=async()=>{
       ...
       let data=this.state.hair_arr
       let id=''
        for(let i=0; i<data.length; i++)
        {
            if(data[i].hair_color_id==userdata.hair_color_id)
            {
               data[i].status=true
                id= data[i].hair_color_id
            }
            else{
               data[i].status=false
            }
        }
        this.setState({hair_arr:data,id:id})
   }
   useralldetaile = async() => {
    ...
    {
        msgProvider.toast(Lang_chg.validationhaircolor[config.language],'center')
        return false
    }
    if(this.state.isConnected==true)
    {
      ...
      data.append('edit_type','hair_color')
     ...
     selectcolor=(index)=>{
         ...
         let data=this.state.hair_arr
           ...
           this.setState({hair_arr:data,id:data[index].hair_color_id})
     ...
     clearselection=()=>{
        ...
        this.setState({hair_arr:data,id:''}) 
     }
    renderitem = ({ item,index }) => {
        if(this.state.hair_arr!='NA')
        {
        return (
            <TouchableOpacity activeOpacity={0.9} onPress={()=>{this.selectcolor(index)}}>
                <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center', borderRadius: 10, borderColor: 'white', borderWidth: 1, paddingHorizontal: 15, marginVertical: 9,width:'100%', height: 52 }}>
                    ...
                    <View>
                        <Text style={{marginRight:25, alignSelf: 'center', fontFamily:'Piazzolla-Bold', color: 'white', fontSize: 16 }}  >{item.hair_color[config.language]}</Text>
    ...
    render() {
        return (
            ...
            <Loader loading={this.state.loading}/>
                  <ScrollView showsVerticalScrollIndicator={false} style={{width:'95%',alignSelf:'center'}}>
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%', }} colors={Colorss.basecolor}>
                    ...
                    <Text style={{  fontSize: 16, fontFamily:'Piazzolla-Bold', color: 'white',width:120,height:30 }}>{Lang_chg.Save[config.language]}</Text>
                        ...
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontFamily:'Piazzolla-Bold', color: 'white' }}>4/10</Text>
                            <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35, resizeMode: 'contain' }} source={require('./icons/hair_colorw.png')}>
                            </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontFamily:'Piazzolla-Bold', color: 'white' }}>{Lang_chg.titlehaircolor[config.language]}</Text>
                    </View>
                    <View style={{marginTop:30}}>
                    {this.state.hair_arr=='NA' &&
