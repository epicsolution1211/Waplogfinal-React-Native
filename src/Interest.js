...
const data1= [
    {
       interest: ['Female','Hembra','Fêmea'],
       interest1: 'women',
       status: false,
       value:1,
    },
    {
        interest: ['Male','Masculina','Masculina'],
        interest1: 'man',
        status: false,
        value:2,
    },
    {
        interest: ['Both','Ambas','Ambas'],
        interest1: 'both',
        status: false,
        value:3,
    },
...
export default class Interest extends Component {
   ...
   userdata1=async()=>{
     ...
     console.log('userdata',userdata)
       let data=this.state.intrest_arr
       let id='';
        for(let i=0; i<data.length; i++)
        {
            if(data[i].interest1==userdata.interested_in)
            {
               data[i].status=true
               id=data[i].value
            }
        ...
        this.setState({intrest_arr:data,id:id})
   }
    useralldetaile = async() => {
        ...
        {
            msgProvider.toast(Lang_chg.validationinterest[config.language],'center')
            return false
        }
        if(this.state.isConnected==true)
        {
          ...
          data.append('edit_type','interested')
    ...
    selectcolor=(index)=>{
         let data=this.state.intrest_arr
          ...
          this.setState({intrest_arr:data,id:data[index].value})
          console.log('data',data)
    }
    clearselection=()=>{
       let data=this.state.intrest_arr
       ...
       this.setState({intrest_arr:data,id:''}) 
    }

    renderitem = ({ item,index }) => {
        console.log('item',item)
      if(this.state.intrest_arr!='NA')
      {
      return (
          <TouchableOpacity activeOpacity={0.9} onPress={()=>{this.selectcolor(index)}}>
              <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center', borderRadius: 10, borderColor: 'white', borderWidth: 1, paddingHorizontal: 15, marginVertical: 9,width:'100%', height: 52 }}>
                  ...
                  <View>
                      <Text style={{marginRight:25, alignSelf: 'center', fontFamily:'Piazzolla-Bold', color: 'white', fontSize: 16 }}>{item.interest[config.language]}</Text>
            ...
            <View style={{ paddingHorizontal: 15}}>
                ...
                <LinearGradient style={{ marginTop: 5, paddingHorizontal: 10, borderRadius: 20,  width: '100%',  }} colors={Colorss.basecolor}>
                    ...
                    <Text style={{  fontSize: 16, fontFamily:'Piazzolla-Bold', color: 'white',width:120,height:30 }}>{Lang_chg.Save[config.language]}</Text>
                        ...
                        <Text style={{ right: 0, position: 'absolute', fontSize: 16, fontFamily:'Piazzolla-Bold', color: 'white' }}>1/10</Text>
                        <Image style={{ marginTop: 22, alignSelf: 'center', height: 35, width: 35, resizeMode: 'contain' }} source={require('./icons/likew.png')}>
                        </Image>
                        <Text style={{ marginTop: 10, alignSelf: 'center', fontSize: 22, fontFamily:'Piazzolla-Bold', color: 'white' }}>{Lang_chg.titleinterest[config.language]}</Text>
                    </View>
                    <View style={{marginTop:60}}>
                        <FlatList
                            data={this.state.intrest_arr}
